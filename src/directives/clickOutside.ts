...
const execHandler = ({ event, handler, middleware }): void => {
  if (middleware(event)) {
    handler(event);
  }
};
const onFauxIframeClick = ({ el, event, handler, middleware }): void => {
...
const onEvent = ({ el, event, handler, middleware }): void => {
...
const beforeMount = (el, { value }): void => {
...
const updated = (el, { value, oldValue }): void => {
